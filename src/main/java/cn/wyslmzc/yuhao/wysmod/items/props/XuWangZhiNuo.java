        ...
        TimerTask timerTask = new TimerTask() {
            ...
            public void run() {
                if (VarInstance.INSTANCE.xuwangzhinuo.get(player) == null) {
                    ...
                    PropUtils.tellRaw(playerWorld, "§5§lw[虚妄之诺]§6伤害结算: 0");
                    return;
                }
                float damage = VarInstance.INSTANCE.xuwangzhinuo.get(player).floatValue();
                if (!player.hurt(DamageSource.MAGIC, damage)) {
                    if (player.getHealth() <= damage) {
                        PropUtils.runCommand(playerWorld, "kill " + player.getName().getContents());
                    }
                }
                PropUtils.tellRaw(playerWorld, "§5§l[虚妄之诺]§6伤害结算: " + damage);
                VarInstance.INSTANCE.xuwangzhinuo.remove(player);
            }
        };
